...
def test_compute_metrics_empty_results() -> None:
    ...
    assert metrics.bpm_n_total_strict == 0
    assert metrics.bpm_mae is None
    assert metrics.bpm_omit_rate is None


def test_compute_metrics_failed_runs() -> None:
    """Test that failed runs are counted correctly."""
    f1 = _make_fixture("test1.wav", bpm_gt=120.0, flags={"bpm_strict"})
    f2 = _make_fixture("test2.wav", bpm_gt=140.0, flags={"bpm_strict"})
    ...
    assert metrics.bpm_n_total_strict == 1
...
def test_compute_metrics_bpm_mae() -> None:
    """Test BPM MAE computation."""
    fixtures = [
        _make_fixture("test1.wav", bpm_gt=120.0, flags={"bpm_strict"}),
        _make_fixture("test2.wav", bpm_gt=140.0, flags={"bpm_strict"}),
        _make_fixture("test3.wav", bpm_gt=100.0, flags={"bpm_strict"}),
    ]
    ...
    assert metrics.bpm_n_total_strict == 3
    assert metrics.bpm_n_predicted == 3
    assert metrics.bpm_n_omitted == 0
    # MAE = (2 + 5 + 1) / 3 = 2.667
    assert metrics.bpm_mae == pytest.approx(2.667, abs=0.01)
    assert metrics.bpm_omit_rate == 0.0
...
def test_compute_metrics_bpm_omissions() -> None:
    ...
    metrics = compute_metrics(results)
    assert metrics.bpm_n_total_strict == 3
    assert metrics.bpm_n_predicted == 2
    assert metrics.bpm_n_omitted == 1
    # MAE only computed on predictions
    assert metrics.bpm_mae == pytest.approx(1.5, abs=0.01)
    # Omit rate = 1/3
    assert metrics.bpm_omit_rate == pytest.approx(0.333, abs=0.01)


def test_compute_metrics_top_bpm_errors() -> None:
    """Test top N BPM errors extraction."""
    fixtures = [_make_fixture(f"test{i}.wav", bpm_gt=120.0, flags={"bpm_strict"}) for i in range(5)]
    ...
    assert len(metrics.top_bpm_errors) == 3

    # Check sorted by error descending
    assert metrics.top_bpm_errors[0].abs_error == 10.0
    assert metrics.top_bpm_errors[1].abs_error == 7.0
    assert metrics.top_bpm_errors[2].abs_error == 5.0
...
def test_compute_metrics_all_omitted() -> None:
    ...
    metrics = compute_metrics(results)
    assert metrics.bpm_n_total_strict == 2
    assert metrics.bpm_n_predicted == 0
    assert metrics.bpm_n_omitted == 2
    assert metrics.bpm_mae is None  # Can't compute MAE with no predictions
    assert metrics.bpm_omit_rate == 1.0
...
def test_metrics_to_json_serializable() -> None:
    ...
    fixtures = [
        _make_fixture("test.wav", bpm_gt=120.0, flags={"bpm_strict"}),
    ]
    results = [
        _make_result(fixtures[0], bpm_value_rounded=122, bpm_omitted=False),
    ]
    ...
    # Check stable keys
    assert parsed["bpm"]["n_total_strict"] == 1
    assert parsed["bpm"]["n_predicted"] == 1
    assert parsed["bpm"]["mae"] == 2.0
    assert parsed["bpm"]["omit_rate"] == 0.0
...
def test_compute_metrics_with_candidates() -> None:
    ...
    f = _make_fixture("test.wav", bpm_gt=120.0, flags={"bpm_strict"})
    ...
    assert len(metrics.top_bpm_errors) == 1

    err = metrics.top_bpm_errors[0]
