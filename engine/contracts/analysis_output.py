...
def _is_uuid_like(v: str) -> bool:
    ...
    if len(parts) != 5:
        return False
    lens = [8, 4, 4, 4, 12]
    return all(len(p) == ln for p, ln in zip(parts, lens, strict=False))
...
def validate_analysis_output_v1(
    obj: dict[str, Any], *, opts: ValidateOptions | None = None
) -> None:
    """
    ...
    for mname, mval in metrics.items():
        ...
        if locked:
            ...
            if (
                "unlock_hint" not in mval
                or not isinstance(mval["unlock_hint"], str)
                or not mval["unlock_hint"].strip()
            ):
                raise _err(
                    path + ".unlock_hint", "locked metrics must include non-empty unlock_hint"
                )
            ...
            if present_forbidden:
                raise _err(
                    path, f"locked metric must not include: {', '.join(sorted(present_forbidden))}"
                )
            ...
            if "value" in mval:
                raise _err(
                    path + ".value",
                    "locked metrics must not include value (use preview if allowed)",
                )
            ...
            if role == "guest":
                ...
                # Guest must not receive numeric confidence except where explicitly allowed
                # (we choose strict: none).
                if "confidence" in mval:
                    raise _err(
                        path + ".confidence", "guest must not receive numeric confidence in v1"
                    )
                if mname == "bpm":
                    val = mval.get("value")
                    if isinstance(val, dict) and "value_exact" in val:
                        raise _err(
                            path + ".value.value_exact", "guest bpm must not include value_exact"
                        )
                ...
                if "candidates" in mval:
                    if mname not in ("bpm", "key_mode"):
                        raise _err(
                            path + ".candidates",
                            "guest candidates only allowed for bpm and key_mode",
                        )
                    _validate_guest_candidates(mval["candidates"], path + ".candidates")

            # Engine v1 says bpm/key_mode MUST be omitted when unreliable.
            # This validator can't compute reliability, but it can prevent them being returned
            # as locked.
